...
function translate(word) { //word by word
  if ("aeiouy".includes(word[0])) {
    return word + "way";
  } else if ("y".includes(word[1]) && word.length == 2) {
    return word + "ay";
  } else if (!"123456789".includes(word[0])) { //if the word is a number, go to line 23, skipping the substr in line 16 (which is a number eater)
      for (var i = 0; i < word.length; i++) { //toggle for selecting the letter of the word
        if (!"aeiouy".includes(word[0])) { //evaluating whether the letter at this position contains one of these letters (vowels) if it does, drop to line 17. otherwise continue to 11
...
exports.translatorModule = translator;
